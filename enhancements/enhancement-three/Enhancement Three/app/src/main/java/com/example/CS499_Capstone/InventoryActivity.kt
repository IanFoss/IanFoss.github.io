    ...
    private fun observeInventory(source: LiveData<List<InventoryItemEntity>>) {
     ...
     override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fabAddItem.setOnClickListener(View.OnClickListener { v: View ->
            ...
            val categoryInput = dialogView.findViewById<EditText>(R.id.edit_category)
            val lowStockThresholdInput = dialogView.findViewById<EditText>(R.id.low_stock_threshold)
            ...
            AlertDialog.Builder(this)
                ...
                .setPositiveButton(
                    ...
                    (DialogInterface.OnClickListener { dialog: DialogInterface, which: Int ->
                        ...
                        itemRepo.checkIfNameExists(name) { exists ->
                            runOnUiThread {
                                if (exists) {
                                    Toast.makeText(this, "Item already exists", Toast.LENGTH_SHORT).show()
                                    return@runOnUiThread
                                }
                                // Input item quantity and validate
                                ...
                                // Input item category and validate
                                var category = ""
                                ...
                                // Input item low stock threshold and validate
                                val lowStockThresholdString = lowStockThresholdInput.text.toString()
                                var lowStockThreshold = 0

                                try {
                                    lowStockThreshold = lowStockThresholdString.toInt()
                                    if (lowStockThreshold < 0) {
                                        throw NumberFormatException()
                                    }
                                } catch (e: NumberFormatException) {
                                    Toast.makeText(this, "Invalid Low Stock Threshold", Toast.LENGTH_SHORT).show()
                                    return@runOnUiThread
                                }

                                // Create new item entity and insert into database using repository
                                val newItem = InventoryItemEntity(name = name,
                                    quantity = quantity,
                                    category = category,
                                    lowStockThreshold = lowStockThreshold)
                                itemRepo.insert(
                                    newItem,
                                    {runOnUiThread { Toast.makeText(this, "Item added", Toast.LENGTH_SHORT).show() }},
                                    {runOnUiThread { Toast.makeText(this, "Item already exists", Toast.LENGTH_SHORT).show() }})
        ...
        AlertDialog.Builder(this)
            ...
            .setItems(options) { _, which ->
                when(which) {
                    ...
                    3 -> observeInventory(itemRepo.getItemsSortedByCategory())
                    4 -> observeInventory(itemRepo.getLowStockItems())
                    5 -> observeInventory(itemRepo.getOutOfStockItems())
                    6 -> observeInventory(itemRepo.allItems)
                }
            }.show()
    }
